...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-outbound-delivery-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(deliveryDocument string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerData, "function": "OutboundDeliveryHeader"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerPartnerData, "function": "OutboundDeliveryHeaderPartner"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": partnerAddressData, "function": "OutboundDeliveryPartnerAddress"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemData, "function": "OutboundDeliveryItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": itemDocumentFlowData, "function": "OutboundDeliveryItemDocumentFlow"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) PartnerAddress(partnerFunction, sDDocument string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": data, "function": "OutboundDeliveryPartnerAddress"})
	if err != nil {
		c.log.Error(err)
		return
	}
